...
export default function Card({item}) {
  ...
  return (
    <Pressable style={{...styles.card, height: height * 0.2}}>
      ...
      <Pressable style={styles.icon} onPress={()=>setIsLiked(prev=>!prev)}><Ionicons name={isLiked?"md-heart":"md-heart-outline"}  size={20} color="#090979" /></Pressable>
    
      <View style={{flexDirection: 'row', alignItems: 'center'}}>
        <Image source={{uri: item.picture}} resizeMode='contain' style={{...styles.cardImg, height: height*0.1}} />
        <View>
     ...
     </Pressable>:<ActivityIndicator size="large" color="#090979" />}
    </Pressable>
  );
}
